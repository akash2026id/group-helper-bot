...
async function cmd(from, thread, text) {
  ...
  if (c === "/help") {
    ...
    await send(thread, "❤️ Love Partner সিস্টেম:\nhttps://singel-c-bot.page.gd");
  } else if (c === "/support") {
    await send(thread, "📞 WhatsApp সাপোর্ট:\nhttps://wa.me/8801966061084");
  } else {
    await send(thread, `❓ "${c}" অজানা কমান্ড। /help লিখুন।`);
  }
}
...
app.get("/webhook", (req, res) => {
  ...
  console.log("VERIFY:", token, "===", VERIFY_TOKEN, "MATCH:", token === VERIFY_TOKEN);
  ...
  for (const e of (req.body.entry || []))
    for (const ev of (e.messaging || []))
      if (ev.message && !ev.message.is_echo && ev.message.text)
        await cmd(ev.sender?.id, ev.recipient?.id, ev.message.text).catch(console.error);
});
